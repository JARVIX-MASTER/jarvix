...
# --- Command Queue Logic ---
COMMAND_FILE_PATH = Path(os.environ.get('TEMP', '')) / 'jarvix_firefox_commands.json'

def poll_command_queue():
    """Background thread to check for commands from Jarvix."""
    while True:
        try:
    ...
    try:
        while True:
            ...
            if message.get("action") == "ping":
                send_message({"status": "ok", "message": "Jarvix Native Host is alive"})
            ...
            # Action: Update Tabs
                ...
                temp_path = Path(os.environ.get('TEMP', '')) / 'jarvix_firefox_tabs.json'
                try:
            ...
            # Action: Save Screenshot
                ...
                if data_url and "base64," in data_url:
                    try:
                        ...
                        shot_path = Path(os.environ.get('TEMP', '')) / 'jarvix_tab_screenshot.png'
        ...
        log_path = Path(os.environ.get('TEMP', '')) / 'jarvix_native_host_error.log'
