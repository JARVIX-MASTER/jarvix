...
BASE_SYSTEM_PROMPT = """
You are Jarvix, a smart laptop assistant with memory.
...
19. Email: {"action": "get_emails"}
    (Triggers: check emails, show inbox, read emails, email summary, my mails, check mail)

20. Upcoming Interviews: {"action": "get_upcoming_interviews"}
    (Triggers: upcoming interviews, interview schedule, when is my interview, my interviews)
...
def process_command(user_input):
    ...
    try:
        ...
        # 11. Force Gmail Commands
        elif any(x in lower for x in ["/emails", "check emails", "show inbox", "read emails", "email summary", "my mails", "check mail"]):
            data = {"action": "get_emails"}
        elif any(x in lower for x in ["/upcoming", "/interviews", "upcoming interviews", "interview schedule", "when is my interview", "my interviews"]):
